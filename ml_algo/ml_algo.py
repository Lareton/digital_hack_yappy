...
def get_res_by_uuid(df: pd.DataFrame, vector_db, model, uuid: str, threshold: float) -> tuple:
    ...
    Описание работы:
    ----------------
    1. Находит индекс текущего видео в датафрейме `df` по UUID.
    2. Загружает видеоролик с указанным UUID и извлекает его вектор признаков.
    3. Выполняет поиск ближайших соседей в базе данных с помощью векторного представления.
    4. Извлекает ближайших соседей, чьи индексы меньше текущего (чтобы исключить текущие или последующие видео).
    5. Сортирует по косинусному сходству и возвращает результат, если ближайший сосед имеет сходство выше заданного порога.
