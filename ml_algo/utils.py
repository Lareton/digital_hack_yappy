...
def download_video(link: str, filename: str) -> None:
    ...
    Описание:
    --------
    1. Выполняется HTTP-запрос на загрузку видео с использованием библиотеки `requests`.
    2. Если запрос успешен, видеофайл сохраняется по указанному пути.
    3. Функция обрабатывает файл по частям (`chunk_size=1024 * 1024`), что позволяет загружать большие файлы без превышения памяти.
    4. Если во время запроса происходит ошибка, она перехватывается и выводится сообщение об ошибке.
